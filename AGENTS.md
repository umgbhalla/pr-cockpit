...
COCKPIT_TAILSCALE_SERVICE=pr-cockpit               # opt-in: advertise svc:pr-cockpit (tagged host required)
COCKPIT_ORIGIN="https://pr-cockpit.tailxxxx.ts.net" # CLI remote origin; never COCKPIT_URL
...
- A phone reaches a hosted cockpit through a mesh VPN whose agent runs on the same host. Tailscale Serve is the opt-in product path: `COCKPIT_TAILSCALE_SERVE=1` keeps the process on `127.0.0.1`, publishes HTTPS on the tailnet, and auto-adds its MagicDNS origin to the origin allowlist. `COCKPIT_TAILSCALE_HTTPS_PORT` selects the HTTPS port and Cockpit refuses to replace an existing root route. `COCKPIT_ORIGIN` makes that tailnet URL primary for both Electron and the CLI. `COCKPIT_TAILSCALE_SERVICE=pr-cockpit` advertises a stable `https://pr-cockpit.<suffix>.ts.net` Service in addition; the host must already be tagged (`tag:server`). User-auth nodes cannot host a Service — log that clearly and leave loopback plus classic Serve running. Serve identity headers (`Tailscale-User-Login` + `Tailscale-Headers-Info`) plus an Origin matching `X-Forwarded-Host` or this tailnet suffix are an extra trusted-browser path; LocalAPI WhoIs is best-effort and often `EACCES` for a user-level process, never a fake success. `update` never leaves `127.0.0.1`. Never use Funnel, never bind `0.0.0.0`, never open TCP 4820. Twingate still uses a Connector on the host with Resource address `127.0.0.1` and TCP restricted to the cockpit port; that alias origin must be added to `COCKPIT_ALLOWED_ORIGINS` or the event socket and every unsafe request return `403`. A new Twingate Resource is visible per identity, so `twingate resources` on one host can omit a Resource that another account already reaches; verify from the account that will use it.
